import { Dt as theme, gt as info, ht as danger, lt as shortenHomePath, v as defaultRuntime } from "./entry.js";
...
import { _ as isBlockedObjectKey } from "./session-key-DIABCrXz.js";
...
import { t as redactConfigObject } from "./redact-snapshot-BErSU9Ln.js";
...
function registerConfigCli(program) {
	const cmd = program.command("config").description("Non-interactive config helpers (get/set/unset). Run without subcommand for the setup wizard.").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.openclaw.ai/cli/config")}\n`).option("--section <section>", "Configure wizard sections (repeatable). Use with no subcommand.", (value, previous) => [...previous, value], []).action(async (opts) => {
		const { configureCommandFromSectionsArg } = await import("./configure-gAyipt5h.js").then((n) => n.t);
